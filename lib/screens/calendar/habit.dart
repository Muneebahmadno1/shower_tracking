...
import 'package:too_cool/screens/calendar/one_day.dart';
import 'package:too_cool/screens/calendar/one_day_button.dart';
...
class HabitState extends State<Habit> {
  ...
  void initState() {
    super.initState();
    _actualMonth = "${months[DateTime.now().month]} ${DateTime.now().year}";
    _updateLastStreak();
  }
              ...
              child: Row(
                ...
                children: [
                  GestureDetector(
                      onTap: () {
                        setState(() {
                          setSelectedDay(Jiffy.parse(_selectedDay.toString())
                              .subtract(months: 1)
                              .dateTime);
                        });
                      ...
                      onTap: () {
                        setState(() {
                          _selectedDay == DateTime.now()
                              ? null
                              : setSelectedDay(
                                  Jiffy.parse(_selectedDay.toString())
                                      .add(months: 1)
                                      .dateTime);
                        });
                        print(_selectedDay.toString());
              ...
              child: TableCalendar(
                ...
                calendarBuilders: CalendarBuilders(
                  defaultBuilder: (context, date, _) {
                    bool selectedColor = homeController.isEventDay(date);
                    return GestureDetector(
                      ...
                      child: Obx(
                        () {
                          print(homeController.eventDates);
                          return OneDayButton(
                            callback: refresh,
                            parent: this,
                            id: widget.habitData.id!,
                            date: date,
                            color: homeController.isEventDay(date)
                                ? HexColor('#83BCFF')
                                : Colors.white,
